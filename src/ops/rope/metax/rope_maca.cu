...
__global__ void rope_kernel(T *out, const T *in, const int64_t *pos_ids,
    ...
    float a = maca_cast<float>(in[base + i]);
    float b = maca_cast<float>(in[base + i + half]);

    out[base + i]        = maca_cast<T>(a * cos_val - b * sin_val);
    out[base + i + half] = maca_cast<T>(b * cos_val + a * sin_val);
}
