#include "self_attention_maca.cuh"
...
__global__ void self_attention_kernel(
    ...
    for (size_t j = tid; j < kv_len; j += block_size) {
        if (static_cast<int>(j) > static_cast<int>(i) + offset) {
            ...
            for (size_t d = 0; d < head_dim; d++)
                dot += maca_cast<float>(q_ptr[d]) * maca_cast<float>(k_ptr[d]);
            score[j] = dot * scale;
        }
    }
    __syncthreads();
    ...
    for (size_t d = tid; d < head_dim; d += block_size) {
        ...
        for (size_t j = 0; j < kv_len; j++) {
            ...
            val += score[j] * maca_cast<float>(v_ptr[d]);
        }
        out_ptr[d] = maca_cast<T>(val);
    }
}
...
void self_attention(std::byte *attn_val, const std::byte *q, const std::byte *k, const std::byte *v,
    ...
    case LLAISYS_DTYPE_F32:
        self_attention_kernel<<<grid_size, block_size, shared_mem, (mcStream_t)stream>>>(
    ...
    case LLAISYS_DTYPE_BF16:
        self_attention_kernel<<<grid_size, block_size, shared_mem, (mcStream_t)stream>>>(
            reinterpret_cast<__maca_bfloat16 *>(attn_val),
            reinterpret_cast<const __maca_bfloat16 *>(q),
            reinterpret_cast<const __maca_bfloat16 *>(k),
            reinterpret_cast<const __maca_bfloat16 *>(v),
            query_len, kv_len, num_heads, num_kv_heads, head_dim, scale);
        return;
    default:
        break;
