...
// 阶段 1：每个 block 用 grid-stride loop 找局部最大值
template <typename T>
...
void argmax(std::byte *max_idx, std::byte *max_val, const std::byte *vals, llaisysDataType_t type, size_t numel, llaisysStream_t stream) {
    ...
    cudaMalloc(&partial_vals, grid_size * sizeof(float));
    cudaMalloc(&partial_idxs, grid_size * sizeof(int64_t));
    ...
    case LLAISYS_DTYPE_BF16:
        argmax_phase1<<<grid_size, BLOCK_SIZE, 0, s>>>(
            partial_vals, partial_idxs, reinterpret_cast<const __nv_bfloat16 *>(vals), numel);
    ...
    cudaFree(partial_vals);
    cudaFree(partial_idxs);
}
}
